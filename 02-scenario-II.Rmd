...
$\delta_{\text} = 0.0$ and condition the prior to compute expected power.
...
that fulfills the power constraint at the prior mean $\delta = 0.4$.
...
tbl_designs <- tibble(
    ...
    initial = list(
        ...
        TwoStageDesign(rpact_design(0.4, 0.025, 0.8, TRUE, order))) )
```
...
is computed both via `evaluate()` and simulation-based.
...
As in [Variant I.2](#variantI.2) another initial design is more appropriate
...
effect sizes ($\delta \leg 0$) should be lower for the optimal design derived
...
As in [Variant II.1](#variantII.1), expected sample size under the full prior 
is minimized.
...
Due to the additional constraint in comparison to [Variant II.1](#variantII.1),
