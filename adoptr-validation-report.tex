...
\CommentTok{# define tolerance function for errors}
\NormalTok{expect_delta_within <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x1, x2, abs_tol) }\KeywordTok{expect_lt}\NormalTok{(}\KeywordTok{abs}\NormalTok{(x1 }\OperatorTok{-}\StringTok{ }\NormalTok{x2), abs_tol)}
...
The optimal design is computed in three variants: as generic two-stage,
group-sequential and one-stage design respectively.
...
\KeywordTok{expect_delta_within}\NormalTok{(}
    ...
    \DataTypeTok{abs_tol =}\NormalTok{ tol_n}
\NormalTok{)}
...
The expected sample size of the optimal designs is simulated and compared
to the outomce of \texttt{adoptr::evaluate()}.
The abs\_tol is set to \(0.5\) what is due to rounding one patient per group
in the worst case.
...
constraint is fullfilled with a relative abs\_tol of \(1\%\).
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-31-1.pdf}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-56-1.pdf}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-107-1.pdf}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-137-1.pdf}
