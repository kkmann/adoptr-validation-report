...
validation of \textbf{\href{https://github.com/kkmann/adoptr}{adoptr}} as much as
possible.
...
Otherwise, the resources linked at \url{https://github.com/kkmann/adoptr} are a good
starting point.
A general overview on adaptive designs is given in \citep{Bauer2015} or much more
detailed in \citep{Wassmer2016}.

\textbf{adoptr} itself already makes extensive use of unittesting to make sure that
all functions work as intended.
Due to constraints on the build-time for an R package, the range of scenarios
covered in the unittests of \textbf{adoptr} is rather limited though.
This report explores a variety of essential scenarios and both formally
tests results wherever possible using \textbf{testthat} \citep{R-testthat} and
provides detailed results printed in the report itself for visual/manual
inspection.
...
An overview of the respective test scenarios is given in the following.
...
\tightlist
\item
  one-stage design respectively
\item
  group-sequential (constant stage-two sample size)
\item
  generic two-stage
\end{enumerate}
...
\CommentTok{# group sequential design:}
\NormalTok{opt1_gs <-}\StringTok{ }\KeywordTok{opt_design}\NormalTok{(init_design_gs)}

\CommentTok{# generic two-stage design:}
\NormalTok{opt1_ts <-}\StringTok{ }\KeywordTok{opt_design}\NormalTok{(}\KeywordTok{TwoStageDesign}\NormalTok{(init_design_gs))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{iters <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(}\KeywordTok{list}\NormalTok{(opt1_ts, opt1_gs, opt1_os), }\ControlFlowTok{function}\NormalTok{(x) x}\OperatorTok{$}\NormalTok{nloptr_return}\OperatorTok{$}\NormalTok{iterations)}
...
\NormalTok{df_toer <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{toer =} \KeywordTok{as.numeric}\NormalTok{(tmp[}\DecValTok{1}\NormalTok{, ]), }\DataTypeTok{se =} \KeywordTok{as.numeric}\NormalTok{(tmp[}\DecValTok{2}\NormalTok{, ]))}
...
\NormalTok{df_pow  <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{pow  =} \KeywordTok{as.numeric}\NormalTok{(tmp[}\DecValTok{1}\NormalTok{, ]), }\DataTypeTok{se   =} \KeywordTok{as.numeric}\NormalTok{(tmp[}\DecValTok{2}\NormalTok{, ]))}
...
\KeywordTok{print}\NormalTok{(df_pow)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##        pow           se
## 1 0.798647 0.0004010114
...
increasing flexibility: one-stage \textgreater{} group-sequential \textgreater{} generic two-stage:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ess_values <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(}
  \KeywordTok{list}\NormalTok{(}\DataTypeTok{OS =}\NormalTok{ opt1_os, }\DataTypeTok{GS =}\NormalTok{ opt1_gs, }\DataTypeTok{TS =}\NormalTok{ opt1_ts), }
  \ControlFlowTok{function}\NormalTok{(x) }\KeywordTok{evaluate}\NormalTok{(ess, x}\OperatorTok{$}\NormalTok{design))}
\NormalTok{testthat}\OperatorTok{::}\KeywordTok{expect_true}\NormalTok{(}\KeywordTok{all}\NormalTok{(}\KeywordTok{diff}\NormalTok{(ess_values) }\OperatorTok{<=}\StringTok{ }\DecValTok{0}\NormalTok{))}
\NormalTok{ess_values}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##       OS       GS       TS 
## 98.00000 80.89317 79.62139
\end{verbatim}

Check simulated sample size vs.~computed:
The expected sample size of the optimal designs is simulated and compared
to the outcome of \texttt{adoptr::evaluate()}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ess_}\DecValTok{0}\NormalTok{ <-}\StringTok{ }\KeywordTok{ExpectedSampleSize}\NormalTok{(datadist, H_}\DecValTok{0}\NormalTok{)}
\NormalTok{ess_}\DecValTok{0}\NormalTok{_values <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(}
  \KeywordTok{list}\NormalTok{(}\DataTypeTok{OS =}\NormalTok{ opt1_os, }\DataTypeTok{GS =}\NormalTok{ opt1_gs, }\DataTypeTok{TS =}\NormalTok{ opt1_ts), }
  \ControlFlowTok{function}\NormalTok{(x) }\KeywordTok{evaluate}\NormalTok{(ess_}\DecValTok{0}\NormalTok{, x}\OperatorTok{$}\NormalTok{design))}
\KeywordTok{expect_equal}\NormalTok{(}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{init_design_}\DecValTok{2}\NormalTok{ <-}\StringTok{ }\KeywordTok{TwoStageDesign}\NormalTok{(init_design_gs)}
...
\NormalTok{testthat}\OperatorTok{::}\KeywordTok{expect_gte}\NormalTok{(}
    \KeywordTok{evaluate}\NormalTok{(ess_}\DecValTok{0}\NormalTok{, }\KeywordTok{TwoStageDesign}\NormalTok{(init_design_gs)),}
...
\NormalTok{            cp_cnstr}
\NormalTok{        ),}
        \DataTypeTok{initial_design =} \KeywordTok{TwoStageDesign}\NormalTok{(init_design_gs),}
        \DataTypeTok{opts =}\NormalTok{ opts}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-30-1.pdf}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-55-1.pdf}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-106-1.pdf}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-126-1.pdf}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-131-1.pdf}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-136-1.pdf}
