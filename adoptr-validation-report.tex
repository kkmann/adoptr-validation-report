...
## 1 one-stage        <OnStgDsg> <named list [3]>  98      98    98       98  
## 2 group-sequential <GrpSqntD> <named list [3]>  80.9    80.9  68.5     68.5
## 3 two-stage        <TwStgDsg> <named list [3]>  79.6    79.7  68.9     68.9
\end{verbatim}
...
## 1 one-stage        <OnStgDsg> <named list [3]>  165   165       165 
## 2 group-sequential <GrpSqntD> <named list [3]>  115.  110.      110.
## 3 two-stage        <TwStgDsg> <named list [3]>  113.  114.      114.
\end{verbatim}
...
boundaries for early efficacy stopping we impose as fairly liberal upper bound
...
## 1 one-stage        <OnStgDsg> <named list [3]>  392     392 
## 2 group-sequential <GrpSqntD> <named list [3]>  324.    323.
## 3 two-stage        <TwStgDsg> <named list [3]>  319.    319.
\end{verbatim}
...
##   type             initial    optimal            ess ess_sim
##   <chr>            <list>     <list>           <dbl>   <dbl>
## 1 one-stage        <OnStgDsg> <named list [3]>  525     525 
## 2 group-sequential <GrpSqntD> <named list [3]>  405.    405.
## 3 two-stage        <TwStgDsg> <named list [3]>  397.    397.
\end{verbatim}
...
##   type             initial    optimal            ess ess_sim
##   <chr>            <list>     <list>           <dbl>   <dbl>
## 1 one-stage        <OnStgDsg> <named list [3]>  428     428 
## 2 group-sequential <GrpSqntD> <named list [3]>  326.    325.
## 3 two-stage        <TwStgDsg> <named list [3]>  319.    319.
\end{verbatim}
...
\NormalTok{      order, }
      ...
      \DataTypeTok{toer_sim  =}\NormalTok{ purrr}\OperatorTok{::}\KeywordTok{map}\NormalTok{(opt}\OperatorTok{$}\NormalTok{design, }
                         \OperatorTok{~}\KeywordTok{sim_pr_reject}\NormalTok{(.[[}\DecValTok{1}\NormalTok{]], }\FloatTok{.0}\NormalTok{, datadist)}\OperatorTok{$}\NormalTok{prob), }
      ...
      \DataTypeTok{power_sim =}\NormalTok{ purrr}\OperatorTok{::}\KeywordTok{map}\NormalTok{(opt}\OperatorTok{$}\NormalTok{design, }
...
\begin{verbatim}
## # A tibble: 3 x 5
##   order   toer power toer_sim power_sim
##   <dbl>  <dbl> <dbl>    <dbl>     <dbl>
## 1     5 0.0250 0.800   0.0250     0.800
## 2     8 0.0250 0.800   0.0250     0.800
## 3    11 0.0250 0.800   0.0250     0.800
\end{verbatim}
...
\begin{verbatim}
## # A tibble: 2 x 5
##   lambda   toer power toer_sim power_sim
##    <dbl>  <dbl> <dbl>    <dbl>     <dbl>
## 1    100 0.0250 0.523   0.0250     0.520
## 2    200 0.0251 0.897   0.0250     0.897
\end{verbatim}

Finally, the three designs computed so far are plotted together to allow
comparison.

\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-86-1.pdf}
...
##   lambda   toer power toer_sim power_sim
##    <dbl>  <dbl> <dbl>    <dbl>     <dbl>
## 1   0.05 0.0250 0.800   0.0250     0.801
## 2   0.2  0.0250 0.800   0.0250     0.800
\end{verbatim}
...
\includegraphics{adoptr-validation-report_files/figure-latex/unnamed-chunk-91-1.pdf}
...
This can be done by means of the \texttt{AverageN2()}-function.
...
\NormalTok{ub_design}\OperatorTok{@}\NormalTok{c2_pivots <-}\StringTok{ }\KeywordTok{rep}\NormalTok{(}\DecValTok{3}\NormalTok{, order)}
\end{Highlighting}
\end{Shaded}
...
##   lambda   toer power toer_sim power_sim
##    <dbl>  <dbl> <dbl>    <dbl>     <dbl>
## 1   0.01 0.0250 0.800   0.0250     0.802
## 2   0.1  0.0250 0.800   0.0250     0.801
\end{verbatim}
...
averange \(n_2\) than the optimal penalized design with \(\lambda = 0.01\)
