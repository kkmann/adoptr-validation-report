...
The expected sample size of the optimal designs is simulated and compared
to the outomce of `adoptr::evaluate()`.
The abs_tol is set to $0.5$ what is due to rounding one patient per group
in the worst case.
```{r}
testthat::expect_delta_within(
...
constraint is fullfilled with a relative abs_tol of $1\%$.
```{r}
cp_sim <- function(z) {
