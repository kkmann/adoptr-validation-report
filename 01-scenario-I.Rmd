...
ess_0_values <- sapply(
  list(OS = opt1_os, GS = opt1_gs, TS = opt1_ts), 
  function(x) evaluate(ess_0, x$design))
expect_equal(
...
```{r}
init_design_2 <- TwoStageDesign(init_design_gs)
...
```{r}
testthat::expect_gte(
    evaluate(ess_0, TwoStageDesign(init_design_gs)),
...
opt3_ts <- minimize(
        ...
        initial_design = TwoStageDesign(init_design_gs),
        opts = opts
)
```
